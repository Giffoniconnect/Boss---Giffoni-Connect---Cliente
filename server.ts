...
// Cron job for cleaning up old preview documents (runs every hour)
setInterval(async () => {
  try {
    console.log("[CRON] Starting cleanup of expired Google Docs previews...");
    const response = await fetch(\`http://localhost:\${PORT}/api/google-docs/cleanup-expired-previews\`, {
      method: "POST"
    });
    if (response.ok) {
      const data = await response.json();
      console.log(\`[CRON] Cleanup completed. Removed \${data.deletedCount} preview files.\`);
    } else {
      console.error("[CRON] Cleanup endpoint failed:", response.status);
    }
  } catch (err: any) {
    console.error("[CRON] Cleanup job error:", err.message);
  }
}, 60 * 60 * 1000);
